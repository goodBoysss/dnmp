...
if [ "${PHP_EXTENSIONS}" != "" ]; then
    apk --update add --no-cache --virtual .build-deps autoconf g++ libtool make curl-dev gettext-dev linux-headers openssh-client
else
  if [ ${1} != "" ]; then
      export PHP_EXTENSIONS="${1}"
  fi
fi
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    #swoole need libstdc++
    apk add --no-cache postgresql-libs libgcc libstdc++ openssl
